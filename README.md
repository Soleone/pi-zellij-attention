...
- `agent_end` → `completed`
- `agent_end` after tool error → `waiting`
...
The extension treats normal status pipe delivery as best-effort and fire-and-forget. Shutdown/unwatch uses a short synchronous send so the idle icon is removed more reliably when pi exits. Repeated identical states are skipped to reduce unnecessary tab renames/flicker.
